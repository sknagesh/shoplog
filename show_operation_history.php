...
$query="SELECT Machine_Name,Operation_ID,prod.Drawing_ID,Start_Date_Time,End_Date_Time,Component_Name,Drawing_NO,
		 Program_NO FROM Production AS prod
...
print("<tr class=\"t\"><th>Machine</th><th>Drawing No and Name</th><th>Program NO</th><th>Start Date and Time</th><th>End Date and Time</th></tr>");
while($row=mysql_fetch_assoc($res))
{

print("<tr class=\"$c\"><td>$row[Machine_Name]</td><td>$row[Drawing_NO]  $row[Component_Name]</td><td>$row[Program_NO]</td><td>$row[Start_Date_Time]</td><td>$row[End_Date_Time]</td></tr>");
	if($c=="q"){$c="s";}else{$c="q";}
}
print("</table>");
}
