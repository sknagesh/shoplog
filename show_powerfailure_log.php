...
if($sdate!=""&&$edate!="")
{
$query="SELECT Operstor_Name,Remarks,DATE_FORMAT(Start_Date_Time,'%d/%m/%Y %h:%i %p') as sdt,
		DATE_FORMAT(End_Date_Time,'%d/%m/%Y %h:%i %p')as edt,
		TIMEDIFF(End_Date_Time,Start_Date_Time) as td FROM ActivityLog as actl
		 INNER JOIN Operator as ope ON ope.Operator_ID=actl.Operator_ID  
 		 WHERE Activity_ID='7' AND Machine_ID=1 
...
{
	$query="SELECT Operator_Name,Remarks,DATE_FORMAT(Start_Date_Time,'%d/%m/%Y %h:%i %p') as sdt,
		DATE_FORMAT(End_Date_Time,'%d/%m/%Y %h:%i %p')as edt,
		TIMEDIFF(End_Date_Time,Start_Date_Time) as td FROM ActivityLog as actl
		 INNER JOIN Operator as ope ON ope.Operator_ID=actl.Operator_ID WHERE Activity_ID='7' AND Machine_ID=1 ORDER BY End_Date_Time DESC LIMIT 10;";
...
print("<tr class=\"t\" ><th>Start Date Time</th><th>End Date Time</th>
		<th>Total Time</th><th>Name</th><th>Remarks</th></tr>");
while ($row = mysql_fetch_assoc($resa))
{
print("<tr class=\"$c\"><td>$row[sdt]</td><td>$row[edt]</td><td>$row[td]</td><td>$row[Operator_Name]</td><td>$row[Remarks]</td></tr>");
if($c=="q"){$c="s";}else{$c="q";}
}
print("</table>");
